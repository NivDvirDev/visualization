function  record15_LEF(SongTag,Fs,y,FreqIndex,Freq,RoundFreqIndex,RoundFreq)
...
% cDisplaySize=[10518, 14310];
% phaseRes=25;
% Real=(-pi):(2*pi/phaseRes):(pi);
% for Real_i=1:phaseRes
%     preh=((rot90((-pi):(2*pi/m):(pi)))*(Freq)).';
%     GPU_preh=gpuArray(exp(complex(Real(Real_i),preh)).*mask);
%     [cDisplayPreh] = CreateStandard_cDisplay3AMP(m,m_real,TotalSample,DetectNumber,SongTag,AMP,Freq,mask);
%     cDisplayPreh=cDisplayPreh./phaseRes;
%     save(fullfile('N:\mat\Project\Mesh',  ['cDisplay_part_',num2str(Real_i),'of',num2str(phaseRes),'.mat']),'cDisplayPreh');
%     if (Real_i==1); cDisplaySize=size(cDisplayPreh); end
%     clear cDisplayPreh;
% end
% 
% cDisplay3AMP=zeros(cDisplaySize);
% 
% for Real_i=1:phaseRes
%     myFileFullName = fullfile('N:\mat\Project\Mesh',['cDisplay_part_',num2str(Real_i),'of',num2str(phaseRes),'.mat']);
%     %[cDisplayPreh]=load(fullMatFileName,'cDisplayPreh' ).cDisplayPreh;
%     matObj=matfile(myFileFullName);
%     cDisplay3AMP=cDisplay3AMP+matObj.cDisplayPreh; clear matObj;
%  
% end
...
% cDisplayPreh=some(cDisplay(Real_i));
% save(fullfile('N:\mat\Project\Mesh',  ['cDisplay_',SongTag,'.mat']),'cDisplayPreh');
% [cepstrum]=CreatePitch_cDisplay3AMP(TotalSample,DetectNumber,SongTag,AMP);
[cDisplay3AMP]=CreateStandard_cDisplay3AMP(m,m_real,TotalSample,DetectNumber,SongTag,AMP,Freq,mask);

%CreateWeirdCircumference(m,m_real,TotalSample,TotalFrameNumber,FreqIndex,folder,T,TBitNum);
...
function [cDisplay3AMP]=CreateStandard_cDisplay3AMP(m,m_real,TotalSample,DetectNumber,SongTag,AMP,Freq,mask)
     global cDisplay3AMP y GPU_preh
     
     preh=((rot90((-pi):(2*pi/m):(pi)))*(Freq)).';
%      complex_preh=gpuArray(exp(complex(0,preh)));
...
%         for Real=(-pi):(2*pi/7):(pi)
%             GPU_g_loop=(gpuArray(exp(complex(Real,preh)).*mask)*GPU_g); 
%             GPU_preh=gpuArray(exp(complex(Real,preh)).*mask);
...
function [cepstrum]=CreatePitch_cDisplay3AMP(TotalSample,DetectNumber,SongTag,AMP)
     global cDisplay3AMP Freq FreqIndex GPU_preh
     cepstrum(size(cDisplay3AMP,1),length(cDisplay3AMP))=0;
     Mel=hz2mel(Freq);
     
     dFreq=([0;diff(Freq)'])';
     %array=(-pi):(2*pi/FreqIndex):(pi);
     array = normalize(Freq,'range',[-pi pi]);
     array=rot90(array);
     preh=((array)*(Freq)).';
     GPU_preh=gpuArray(exp(complex((pi)/2,preh)));
     
     
     for sample=1:size(cDisplay3AMP,2)
        tic
        for i=1:1:DetectNumber
            g(:,i)=cDisplay3AMP(:,sample);
        end
        GPU_g=gpuArray(g); clear g;

        GPU_g=(GPU_preh*GPU_g);
        deliver=  gather(sum(abs(GPU_g),2))  ; clear GPU_g;
        spiral(:)=deliver(:).*dFreq(:);    clear deliver;
        cepstrum(:,sample)=spiral; clear spiral;
    ...
    %  [meanSpiral]=printcycle(spiral,meanSpiral,sample,Freq,RoundFreq,RoundFreqIndex,Round_Y);
     ...
     save(fullfile('N:\mat\Project\Mesh',  ['cDisplay_',SongTag,'.mat']),'cDisplay3AMP','cepstrum');
end

function CreateWeirdCircumference(m,m_real,TotalSample,TotalFrameNumber,FreqIndex,folder,T,TBitNum)
    reset(gpuDevice(1));
    % yg = gpuArray(complex(mean(y,2),0));
    global FWF gCircel InerCircel y  cDisplay3AMP
    ...
    for Precord=1:1:(length(packet)-1)
            if ((mod(Precord,10))==1)  
                ...
                clear matObj;
                load(fullMatFileName, 'cDisplay3PHAZ');  save(fullMatFileName, 'cDisplay3PHAZ');
                matObj = matfile(fullMatFileName,'Writable',true);
            ...
            try
                ...
                switch ME.identifier
                    case 'MATLAB:complex:invalidRealPartInput'
                        disp(['ME.identifier: ',ME.identifier]);
            ...
            if (ndims(zeros(pack,FreqIndex,InerCircel))~=ndims(Packet.data)); ...
                        FWF=gpuArray(zeros(pack,FreqIndex,InerCircel)); end
    ...
    %         save(fullMatFileName,'cDisplay3PHAZ');
            ...
            clear Packet;
            time = toc;    disp(['************load the file: in ',num2str(time),'seconds ************']);

    end

    for Precord=1:10:(length(packet)-1)
        fullMatFileName = fullfile(folder,['myFile',sprintf('%03i',Precord),'.mat']);
        tic;    load(fullMatFileName, 'cDisplay3PHAZ');    time = toc;
        disp(['************Save the file ',fullMatFileName,' in ',num2str(time),'seconds ************']);
        tic;    save(fullMatFileName, 'cDisplay3PHAZ');    time = toc;
        disp(['************Save the file ',fullMatFileName,' in ',num2str(time),'seconds ************']);
    end
 ...
 function printcycle(T,TBitNum,FreqIndex,sample,spiral)     
       ...
       for myfreq=1:FreqIndex
           if (spiral(myfreq)>4)
                    ...
                    FWF(sample,myfreq,:)=interp1( (1:1:dc), meanWave, linspace(1, dc,InerCircel ), 'makima'); 
                    clear meanWave dc;
