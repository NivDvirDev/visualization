#!/usr/bin/env python3
"""
SYNESTHESIA 3.4 - Temporal Memory
=================================
Adds visual continuity through:
1. Alpha blending with previous frames (ghosting/persistence)
2. Feature smoothing (amplitude values smoothed over time)
3. Energy accumulator (builds momentum on intense sections)

Combined with previous features:
- Pitch-driven shapes
- Continuous line melody trail
"""
...
class ContinuousLineTrail(MelodicTrail):
    """Melody trail as continuous line."""
...
class TemporalMemoryRenderer(TemporalSpiralRenderer):
    """
    Renderer with temporal memory for visual continuity.
    """
    ...
    def __init__(self, config, frame_rate=30):
        ...
        # === TEMPORAL MEMORY ===
        # Previous frames for alpha blending
        self.frame_history = deque(maxlen=4)
        
        # Smoothed amplitude (persists between frames)
        self.smoothed_amplitude = None
        self.amplitude_smoothing = 0.35  # 0=no smoothing, 1=frozen
        
        # Energy accumulator
        self.energy_accumulator = 0.0
        self.energy_decay = 0.92  # How fast energy fades
        self.energy_buildup = 0.15  # How fast energy builds
        
        # Frame blending weights (newest to oldest)
        self.blend_weights = [0.55, 0.25, 0.12, 0.08]
    ...
    def _smooth_amplitude(self, amplitude_data):
        """Apply temporal smoothing to amplitude data."""
        if self.smoothed_amplitude is None or len(self.smoothed_amplitude) != len(amplitude_data):
            self.smoothed_amplitude = amplitude_data.copy()
        else:
            # Exponential moving average
            self.smoothed_amplitude = (
                self.amplitude_smoothing * self.smoothed_amplitude +
                (1 - self.amplitude_smoothing) * amplitude_data
            )
        return self.smoothed_amplitude.copy()

    def _update_energy(self, amplitude_data):
        """Update energy accumulator based on current frame energy."""
        current_energy = np.mean(amplitude_data ** 2)
        
        # Energy builds up on loud sections, decays otherwise
        if current_energy > self.energy_accumulator:
            self.energy_accumulator = (
                self.energy_accumulator * (1 - self.energy_buildup) +
                current_energy * self.energy_buildup
            )
        else:
            self.energy_accumulator *= self.energy_decay
        
        return self.energy_accumulator

    def _blend_with_history(self, current_frame):
        """Blend current frame with previous frames for persistence effect."""
        if len(self.frame_history) == 0:
            self.frame_history.append(current_frame.copy())
            return current_frame
        
        # Start with weighted current frame
        result = current_frame.copy()
        
        # Convert to float for blending
        result_arr = np.array(result, dtype=np.float32)
        total_weight = self.blend_weights[0]
        result_arr *= self.blend_weights[0]
        
        # Add previous frames with decreasing weights
        for i, prev_frame in enumerate(reversed(list(self.frame_history))):
            if i + 1 >= len(self.blend_weights):
                break
            weight = self.blend_weights[i + 1]
            prev_arr = np.array(prev_frame, dtype=np.float32)
            result_arr += prev_arr * weight
            total_weight += weight
        
        # Normalize
        result_arr /= total_weight
        result_arr = np.clip(result_arr, 0, 255).astype(np.uint8)
        
        blended = Image.fromarray(result_arr, 'RGB')
    ...
    def render_frame(self, amplitude_data, frame_idx=0, frequencies=None,
        ...
        # === TEMPORAL MEMORY: Smooth amplitude ===
        smoothed_amp = self._smooth_amplitude(amplitude_data)
        
        # === TEMPORAL MEMORY: Update energy ===
        energy = self._update_energy(amplitude_data)
        # Energy affects overall brightness and scale
        energy_boost = min(0.3, energy * 2)

        img = self._create_radial_gradient_background(background_color)
        draw = ImageDraw.Draw(img)

        effective_scale = pulse_scale * atmos_effects['particle_scale'] * (1 + energy_boost * 0.2)

        # Render spiral with SMOOTHED amplitude
        self._render_pitch_spiral(draw, smoothed_amp, effective_scale, 
                                   pulse_brightness + energy_boost,
        ...
        # === TEMPORAL MEMORY: Blend with previous frames ===
        img = self._blend_with_history(img)
...
def main():
    ...
    output_path = "/Users/guydvir/Project/04_Code/synesthesia2/synth34_temporal_memory.mp4"

    print("=" * 70)
    print("SYNESTHESIA 3.4 - Temporal Memory")
    print("=" * 70)
    print("New features:")
    print("  ✓ Frame blending (persistence/ghosting effect)")
    print("  ✓ Amplitude smoothing (no sudden jumps)")
    print("  ✓ Energy accumulator (builds momentum)")
    print()
    print("Previous features:")
    print("  ✓ Pitch-driven shapes")
    print("  ✓ Continuous line melody trail")
    print()
    ...
    generator.renderer = TemporalMemoryRenderer(render_config, config.frame_rate)
