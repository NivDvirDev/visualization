#!/usr/bin/env python3
"""
SYNESTHESIA - Unified Merkabah Video Generator
================================================
Generate videos with the unified heaven-earth visualization where
the Star Tetrahedron emerges from curving planes.
"""
...
from merkabah_3d_unified import UnifiedMerkabah3DRenderer, UnifiedMerkabah3DConfig


def generate_unified_merkabah_video(audio_path: str, output_path: str, duration: float = 30,
                                     glow_radius: int = 8, plane_curvature: float = 0.8):
    """Generate a unified Merkabah video with heaven-earth planes."""

    print("=" * 65)
    print("SYNESTHESIA - Unified Merkabah Video Generator")
    print("  'Heaven and Earth reaching toward each other'")
    print("=" * 65)
    print(f"\nInput: {audio_path}")
    print(f"Output: {output_path}")
    print(f"Duration: {duration}s")
    print(f"Settings: glow_radius={glow_radius}, curvature={plane_curvature}")
    sys.stdout.flush()

    # Create temp directory
    temp_dir = tempfile.mkdtemp(prefix="merkabah_unified_")
    ...
    print("\n[2/4] Creating unified Merkabah renderer...")
    sys.stdout.flush()

    config = UnifiedMerkabah3DConfig(
        frame_width=1280,
        frame_height=720,
        glow_radius=glow_radius,
        plane_curvature=plane_curvature,
        enable_stars=True,
        enable_fire=True,
        enable_chayot=True,
        num_stars=150
    )
    renderer = UnifiedMerkabah3DRenderer(config)
    print("  ✓ Unified renderer initialized")
    print("    - Sky and Earth planes curve toward center")
    print("    - Star Tetrahedron emerges from plane intersection")
    print("    - Ophanim, Chayot, Eyes, Fire, Lightning enabled")
    sys.stdout.flush()
    ...
    print(f"\n[3/4] Rendering {total_frames} unified frames...")
    sys.stdout.flush()
    ...
    if result.returncode != 0:
        ...
        # Try encoding without audio merge issues
        print("  Retrying with explicit audio duration...")
        ffmpeg_cmd_retry = [
            'ffmpeg', '-y',
            '-framerate', '30',
            '-i', os.path.join(frames_dir, 'frame_%06d.png'),
            '-i', audio_path,
            '-map', '0:v', '-map', '1:a',
            '-t', str(min(duration, total_frames / 30)),
            '-c:v', 'libx264',
            '-preset', 'medium',
            '-crf', '18',
            '-c:a', 'aac',
        ...
        subprocess.run(ffmpeg_cmd_retry, check=True, capture_output=True)
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Generate unified Merkabah video')
    parser.add_argument('input', help='Input audio file')
    parser.add_argument('-o', '--output', default='merkabah_unified_output.mp4')
    ...
    parser.add_argument('--curvature', type=float, default=0.8, help='Plane curvature (0-1)')
    args = parser.parse_args()

    generate_unified_merkabah_video(
        args.input,
        args.output,
        args.duration,
        glow_radius=args.glow,
        plane_curvature=args.curvature
    )
