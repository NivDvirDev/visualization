...
class FastSpiralRenderer:
    ...
    def render_frame(self,
                     ...
                     wave_enabled: bool = True) -> Image.Image:
        """
        ...
        x_rot = self.base_x * cos_a - self.base_y * sin_a + self.config.center_x
        y_rot = self.base_x * sin_a + self.base_y * cos_a + self.config.center_y
        ...
        # Draw circles from outer to inner (so inner overlaps outer)
        for i in range(self.config.num_frequency_bins - 1, -1, -1):
            x = int(x_rot[i])
            y = int(y_rot[i])

            # Circle size based on amplitude and wave
